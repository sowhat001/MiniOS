...
void buddy_info() {
    ...
    for (index = 0; index <= MAX_BUDDY_ORDER; ++index) {
        kernel_printf("\tOrder %d: %d frees\n", index, buddy.freelist[index].nr_free);
        freeSize+=buddy.freelist[index].nr_free*(1<<index);
    }
    kernel_printf("\tfree page-frame number : %d\n", freeSize);
}
...
void init_pages(unsigned int start_pfn, unsigned int end_pfn) {
    ...
    for (i = start_pfn; i < end_pfn; i++) {
        ...
        (pages + i)->bplevel = 0;
        (pages + i)->freelist = 0;  // initially, the free space is the whole page
...
found:
    ...
    while (current_order > bplevel) {
        ...
        set_flag(buddy_page, _PAGE_FREE);
    }
...
// free the page at addr with bplevel
